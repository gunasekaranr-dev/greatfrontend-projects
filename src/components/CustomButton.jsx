    ...
    return (
        <div className='position-relative vh-100'>
            <div className='w-75 position-absolute top-50 start-50 translate-middle'>
                {CustomButtonContent.map((btn, index) => (
                    btn.rows.map((row, rowIndex) => (
                        <Button key={rowIndex} variant={row.varient} size={row.size} style={{ backgroundColor: row.backgroundColorCode, color: row.textColorCode }} className='w-auto border-0 rounded-1 mx-5 my-1 p-2'>
